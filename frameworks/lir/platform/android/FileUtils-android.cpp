...
FileUtils* FileUtils::getInstance()
{
    if (s_sharedFileUtils == nullptr)
    {
        ...
        if (!s_sharedFileUtils->init())
        {
          ...
          LLOG("ERROR: Could not init CCFileUtilsAndroid");
        }
    }
...
bool FileUtilsAndroid::isDirectoryExistInternal(const std::string& dirPath) const
{
    ...
    if (s[0] == '/')
    {
        LLOG("find in flash memory dirPath(%s)", s);
        struct stat st;
        if (stat(s, &st) == 0)
        {
...
FileStatus FileUtilsAndroid::getContents(const std::string& filename, ResizableBuffer* buffer)
{
