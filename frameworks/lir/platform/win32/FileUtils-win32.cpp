...
FileUtils* FileUtils::getInstance()
{
    if (s_sharedFileUtils == nullptr)
    {
        ...
        if(!s_sharedFileUtils->init())
        {
          ...
          LIRLOG("ERROR: Could not init CCFileUtilsWin32");
        }
    }
...
FileStatus FileUtilsWin32::getContents(const std::string& filename, Buffer* buffer)
{
    if (filename.empty())
    ...
    if (!successed) {
		LIRLOG("Get data from file(%s) failed, error code is %s", filename.data(), std::to_string(::GetLastError()).data());
...
bool FileUtilsWin32::renameFile(const std::string &oldfullpath, const std::string& newfullpath)
{
    ...
    if (FileUtils::getInstance()->isFileExist(newfullpath))
    {
        ...
        {
            LIRLOGERROR("Fail to delete file %s !Error code is 0x%x", newfullpath.c_str(), GetLastError());
        }
    }
    ...
    {
		LIRLOGERROR("Fail to rename file %s to %s !Error code is 0x%x", oldfullpath.c_str(), newfullpath.c_str(), GetLastError());
        return false;
    }
}
    ...
    if ((GetFileAttributes(path.c_str())) == INVALID_FILE_ATTRIBUTES)
    {
        ...
        for (unsigned int i = 0, size = dirs.size(); i < size; ++i)
        {
            ...
            if (!isDirectoryExist(utf8Path))
            {
                ...
                {
                    LIRLOGERROR("Fail create directory %s !Error code is 0x%x", utf8Path.c_str(), GetLastError());
                    return false;
                }
            }
        }
...
bool FileUtilsWin32::removeFile(const std::string &filepath)
{
    ...
    {
        LIRLOGERROR("Fail remove file %s !Error code is 0x%x", filepath.c_str(), GetLastError());
        return false;
    }
}
