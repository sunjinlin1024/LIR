...
public:
	...
	ValueMap dictionaryWithContentsOfFile(const std::string& fileName)
	{
		...
		LASSERT(parser.init("UTF-8"), "The file format isn't UTF-8");
	...
	void startElement(void *ctx, const char *name, const char **atts)
	{
		...
		if (sName == "dict")
		{
			...
			else if (SAX_DICT == preState)
			{
				...
				LASSERT(!_dictStack.empty(), "The state is wrong!");
		...
		else if (sName == "array")
		{
			...
			{
				LASSERT(!_arrayStack.empty(), "The state is wrong!");
	...
	void textHandler(void *ctx, const char *ch, size_t len) override
	{
		...
		case SAX_STRING:
		{
						   ...
						   {
							   LASSERT(!_curKey.empty(), "key not found : <integer/real>");
						   }
...
bool FileUtils::writeDataToFile(const Buffer& data, const std::string& fullPath)
{
	size_t size = 0;
	const char* mode = "wb";

	LASSERT(!fullPath.empty() && data.size() != 0, "Invalid parameters.");
...
Buffer FileUtils::getDataFromFile(const std::string& filename)
{
	Buffer d;
...
//		LASSERT(readedSize == 0 || readedSize == (int)fileInfo.uncompressed_size, "the file size is wrong");
//
...
{
	LASSERT(!dirPath.empty(), "Invalid path");
...
//{
//	LASSERT(isAbsolutePath(fullPath), "Async isDirectoryExist only accepts absolute file paths");
//	performOperationOffthread([fullPath]() -> bool {
...
{
	LASSERT(false, "FileUtils not support createDirectory");
	return false;
}

bool FileUtils::removeDirectory(const std::string& path)
{
	LASSERT(false, "FileUtils not support removeDirectory");
	return false;
}

bool FileUtils::removeFile(const std::string &path)
{
	LASSERT(false, "FileUtils not support removeFile");
	return false;
}

bool FileUtils::renameFile(const std::string &oldfullpath, const std::string& newfullpath)
{
	LASSERT(false, "FileUtils not support renameFile");
	return false;
}
...
{
	LASSERT(false, "getFileSize should be override by platform FileUtils");
	return 0;
}
...
bool FileUtils::createDirectory(const std::string& path)
{
	LASSERT(!path.empty(), "Invalid path");
...
bool FileUtils::renameFile(const std::string &oldfullpath, const std::string &newfullpath)
{
	LASSERT(!oldfullpath.empty(), "Invalid path");
	LASSERT(!newfullpath.empty(), "Invalid path");
...
long FileUtils::getFileSize(const std::string &filepath)
{
	LASSERT(!filepath.empty(), "Invalid path");
